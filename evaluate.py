...
def eval_PSNR(args, model, dataset, root):
    """Evaluate masks to produce mAP (and PSNR) scores."""
    root = os.path.join(root, "PSNR")
    os.makedirs(root)
...
def render_single_views(args, model, dataset, root, save_cache=False):
    """Render  separate frames."""
...
def eval_single_masks(args, model, dataset, root):
    """Evaluate masks to produce mAP (and PSNR) scores."""
    root = os.path.join(root, "single_masks")
    os.makedirs(root)

    image_ids = dataset.img_ids

    results = evaluation.evaluate_single(
        dataset,
        model,
        root,
        vis_i=1,
...
def run(args, model, dataset, root):
    ...
    #Mie Funzioni
    if "SingleFrames" in args.outputs:
        render_separate_video(args, model, dataset, root)
    if "SingleMasks" in args.outputs:
        eval_single_masks(args, model, dataset, root)
    if "SingleViews" in args.outputs:
        render_single_views(args, model, dataset, root)
    if "PSNR" in args.outputs:
         # segmentations and renderings and PSNR
        eval_PSNR(args, model, dataset, root)
